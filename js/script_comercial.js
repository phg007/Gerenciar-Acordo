...
const elementSelectComprador = document.getElementById("comprador");
...
$("#aprovarAcordoComercial").on("click", function (a) {
  ...
  if (checkede == 0) {
    ...
    let valorAcordo = $(".check:checked")
      .closest("tr")
      .find(".valorAcordo")
      .toArray()
      .map(function (valorAcordo) {
...
const buscarFornecedor = () => {
  ...
  $.ajax({
    url: "buscarFornecedor.php?",
    method: "get",
    data: "comprador=" + comprador,
    success: function (rel_entrada) {
      $(".atualizarforn").empty().html(rel_entrada);
    },
  });
};

elementSelectComprador.addEventListener("change", () => {
  buscarFornecedor();
});
